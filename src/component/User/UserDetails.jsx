...
const UserDetails = () => {
  ...
  const getdata = async () => {
    const user = userDataDummy?.find((user)=>user?.id == id)
    console.log(user)
    setSingle_user(user)
  }
        ...
        <div className="userList" style={{ marginBottom: 0, borderRadius: '12px 12px 0 0 ' }} >
          ...
          <div style={{ flex: 1 }}>
            <h6 style={{ marginBottom: 10, fontWeight: 600, color: '#3F4254' }}>{single_user?.name}
              ...
              {single_user?.is_gold_member ? <span className='span-box' style={{ background: '#F8F5FF', color: '#5014D0', marginLeft: 10 }}>Pro user</span>:null}
            </h6>
