  ...
  const getdata = async()=>{
    try{
      ...
      const response = await axios({
        method: "get",
        url: `/get_single_user?user_id=${id}`,
        headers:{
      ...
      if(response.status==200){
        ...
        setSingle_user(response?.data?.user)
      }
...
<div className="userList" style={{marginBottom:0,borderRadius:'12px 12px 0 0 '}} >
<img src={single_user?.avatar} style={{width:170,height:170,borderRadius:6}}></img>

<div style={{flex:1}}>
<h6  style={{marginBottom:10,fontWeight:600,color:'#3F4254'}}>{single_user?.name}

{single_user?.kyc_status &&<svg width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg" style={{marginLeft:12}}>
...
{single_user?.is_gold_memeber && <span className='span-box' style={{background:'#F8F5FF',color:'#5014D0',marginLeft:10}}>Pro user</span>}
</h6>
...
</svg>{single_user?.email}
&nbsp;&nbsp;&nbsp;

<span className="span-box" style={{color:'#47BE7D',background:'#E8FFF3'}}>Active</span></p>
<div  className="tournament-type" >

<div>
                <h5 style={{fontWeight:900,color:'#3F4254',marginBottom:0}}>₹{single_user?.winning}</h5>
                <p className="span-text-light" style={{fontSize:13,marginBottom:0}}>Total Earning </p>
            </div>
            <div>
                <h5 style={{fontWeight:900,color:'#3F4254',marginBottom:0}}>{single_user?.followers}</h5>
                <p className="span-text-light" style={{fontSize:13,marginBottom:0}}>Followers</p>
            </div>
            <div>
                <h5 style={{fontWeight:900,color:'#3F4254',marginBottom:0}}>{single_user?.following}</h5>
...
<button className="themeButton" style={{backgroundColor:'#FFF5F8',color:'#D9214E'}}>Delete</button>
<button className="themeButton">Edit</button>

</div>
<div>
<p className='span-text-light'>Profile <span className='span-text-dark float-end' >100%</span></p>
...
<div className="userDetails-tabs "  style={{borderRadius:'0 0 12px 12px'}}>
    <span style={tab==1 ? {color:'#00A3FF',borderBottom: '3px solid #00A3FF'}:{color:'#A1A5B7'}} onClick={()=>setTab(1)}>Recent </span>
    <span style={tab==2 ? {color:'#00A3FF',borderBottom: '3px solid #00A3FF'}:{color:'#A1A5B7'}} onClick={()=>setTab(2)}>General </span>
    <span style={tab==3 ? {color:'#00A3FF',borderBottom: '3px solid #00A3FF'}:{color:'#A1A5B7'}} onClick={()=>setTab(3)}>Withdraw </span>
    <span style={tab==4 ? {color:'#00A3FF',borderBottom: '3px solid #00A3FF'}:{color:'#A1A5B7'}} onClick={()=>setTab(4)}>Cards </span>
    <span style={tab==5 ? {color:'#00A3FF',borderBottom: '3px solid #00A3FF'}:{color:'#A1A5B7'}} onClick={()=>setTab(5)}>Post</span>
    <span style={tab==6 ? {color:'#00A3FF',borderBottom: '3px solid #00A3FF'}:{color:'#A1A5B7'}} onClick={()=>setTab(6)}>Join tournamnent List</span>
    <span style={tab==7 ? {color:'#00A3FF',borderBottom: '3px solid #00A3FF'}:{color:'#A1A5B7'}} onClick={()=>setTab(7)}>tournamnent History</span>
    <span style={tab==8 ? {color:'#00A3FF',borderBottom: '3px solid #00A3FF'}:{color:'#A1A5B7'}} onClick={()=>setTab(8)}>Game Character</span>
    <span style={tab==9 ? {color:'#00A3FF',borderBottom: '3px solid #00A3FF'}:{color:'#A1A5B7'}} onClick={()=>setTab(9)}>Payment</span>
    <span style={tab==10 ? {color:'#00A3FF',borderBottom: '3px solid #00A3FF'}:{color:'#A1A5B7'}} onClick={()=>setTab(10)}>Chat</span>
...
{tab==1 && ( <motion.div  initial={{x:-200}} animate={{ x: 0 }} ><Tab1 /></motion.div>)}

{tab==2 && ( <motion.div  initial={{x:-200}} animate={{ x: 0 }} ><Tab2 single_user={single_user} setSingle_user={setSingle_user} /></motion.div>)}
{tab==3 && ( <motion.div  initial={{x:-200}} animate={{ x: 0 }} ><Tab3 single_user={single_user} /></motion.div>)}

{tab==4 && ( <motion.div  initial={{x:-200}} animate={{ x: 0 }} ><Tab4 /></motion.div>)}

{tab==5 && ( <motion.div  initial={{x:-200}} animate={{ x: 0 }} ><Tab5 single_user={single_user} setSingle_user={setSingle_user}/></motion.div>)}
{tab==6 && ( <motion.div  initial={{x:-200}} animate={{ x: 0 }} ><Tab6 single_user={single_user}/></motion.div>)}

{tab==7 && ( <motion.div  initial={{x:-200}} animate={{ x: 0 }} ><Tab7 /></motion.div>)}
{tab==8 && ( <motion.div  initial={{x:-200}} animate={{ x: 0 }} ><Tab8 single_user={single_user}/></motion.div>)}

{tab==9 && ( <motion.div  initial={{x:-200}} animate={{ x: 0 }} ><Tab9  single_user={single_user}/></motion.div>)}
{tab==10 && ( <motion.div  initial={{x:-200}} animate={{ x: 0 }} ><Tab10 /></motion.div>)}
