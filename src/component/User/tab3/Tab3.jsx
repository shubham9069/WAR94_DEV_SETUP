  ...
  return (
    <>
      <div className='tabinfo padding15rem'>
        ...
        {userTabData?.tab3.inputArray.map((obj) => {
          return <div className="inputwrapper">
            <p className='span-text-dark' >{obj.label}</p>
            <input className="form-input" value={WithdrawData[obj.name]} onChange={handleChange} name={obj.name} placeholder={`Enter ${obj.label}`}></input>
          </div>
        })}
      ...
      <div className='tabinfo padding15rem' style={{ marginTop: '1.5rem' }}>
        ...
        {userTabData.tab3.paymentHistory.map((element) => {
          ...
          return <div style={{ marginBottom: '2rem' }}>
            ...
            <div className='center-div tab1-right-div' style={{ background: '#F1FAFF', border: '1px dashed #00A3FF' }}>
              ...
              <p style={{ flex: 1, marginBottom: 0, fontWeight: 600, fontSize: 15 }}> {element?.payment_method?.account_name}
