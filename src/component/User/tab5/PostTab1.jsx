...
<div class="dropdown" onClick={(e)=>{seteditPostdata(item);setEditPostText(item?.text);setimage("")}}>
...
{item?.comments?.slice(0,commentSlice[item?.post_id])?.map((comment)=>{
  ...
  <div style={{flex:1}}>
    ...
    <span style={{fontWeight:600}}>{comment.user?.name}</span>
     ...
     <div class="dropdown">
     <span  className=" dropdown-toggle" data-bs-toggle="dropdown" aria-expanded="false" style={{float:'right',color:'#B5B5C3',marginLeft:10,cursor:'pointer'}}>Reply</span>
     <ul class="dropdown-menu post-dropdown " style={{width:'250px'}}  >
