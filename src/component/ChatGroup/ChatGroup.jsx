...
import { groupChat } from '../../dummydata/DummyData'
...
const ChatGroup = () => {
    ...
    return (
        <>
                        ...
                        <table style={{ width: '100%' }} >
                            ...
                            {groupChat?.map((obj) => {
                                ...
                                return <tr>
                                    ...
                                    <td style={{ color: '#3F4254', fontSize: 14 }}>{obj.group_name}</td>

                                    <td style={{ display: 'flex' }}>

                                        {obj.group_member?.map((member, i) => {

                                            return <img src={member.image} style={{ width: 40, height: 40, objectFit: 'cover', borderRadius: 50, position: 'relative', right: 15 * i }}></img>
                                        })}
                                        <div className='center-div' style={{ width: 40, height: 40, objectFit: 'cover', borderRadius: 50, position: 'relative', right: 15 * obj.group_member.length, background: "#181C32", color: "white" }}>#20</div>
                                    </td>
                                    <td style={{ color: '#00A3FF', fontSize: 14, fontWeight: 600 }}>{obj.created_by}</td>

                                    <td style={{ display: 'flex' }}>

                                        {obj.group_admin?.map((admin, i) => {

                                            return <img src={admin.image} style={{ width: 40, height: 40, objectFit: 'cover', borderRadius: 50, position: 'relative', right: 10 * i }}></img>
                                        })}
            ...
            <Modal show={modalChat} onHide={() => setmodalChat(null)} size="lg" style={{ backdropFilter: 'blur(8px)' }}>
                <Modal.Header closeButton>
                    <Modal.Title style={{ fontSize: 18, fontWeight: 600, color: '#3F4254', letterSpacing: 1 }}>{modalChat?.group_name}</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <div >
                        <div className='nameofchat d-flex' style={{ gridGap: 10 }}>
                            ...
                            <div style={{ display: 'flex' }}>
                                ...
                                {modalChat?.group_member?.map((a, i) => {

                                    return <img src={a.image} style={{ width: 40, height: 40, objectFit: 'cover', borderRadius: 50, position: 'relative', right: 10 * i }}></img>
                                })}
