...
const Banner = ({Status_Active_InActive}) => {
    const {userToken,country_list} = useContext(AuthContext)
     const location = useLocation()
    const [BannerArr,setBannerArr] = useState([])
    const [searchCon,setSearchCon] = useState("")
    const [Country,setCountry] = useState([])
    const [eventData,setEventData] = useState({name:"",target:"",status:1,banner:"", });
    const {name,target,status,banner} = eventData
    ...
    const [editEventid,setEditEventid] = useState("")
      ...
      const eventupload = async(e)=>{
        ...
        formData.append("redirection_link",target)
        formData.append("status",status)
        formData.append("banner",banner)
        formData.append("",JSON.stringify(Country))
        formData.append("redirection",0)
        formData.append("item",0)
        formData.append("subitem",0)
        formData.append("subitem2",0)
        ...
        try{
          ...
          }),
          {
           ...
           const newdata = data?.data
            ...
            var arr = BannerArr?.filter(a=>a?.id!=Id)
            setBannerArr(arr)
              return newdata?.message
            }},
            error: {render({data}){
      ...
      formData.append("event_id",editEventid)
        ...
        try{
          ...
          }),
          {
            ...
            success: {render({data}){
           const newdata = data?.data
           var data = BannerArr?.map((element)=>{

            return element?._id == editEventid  ? {
              ...element,          
              status,
  ...
  if(event){
    ...
    var obj={
      name:event.title,
      status:event.status,
      target:event?.redirection_link,
      banner:event?.banner
     
    }
    setCountry(JSON.parse(event?.country))
  ...
  const getData= async()=>{
    ...
    try{
      ...
      const response = await axios({
        method: "get",
        url: `/get_all_banners?status=${Status_Active_InActive}`,
        headers:{
        'Authorization': `Bearer ${userToken} `,
  ...
  <h4 className='section-heading' >All Banners</h4>
  ...
  New Banner</button>
      ...
      <div className='event-container' >
          ...
          <div>
          <span className='span-text-dark' style={{fontSize:14,flex: 1}}>{event?.title}</span>
      ...
      <Modal show={newEvent} onHide={()=>setNewEvent(false)} size='lg'>
          ...
          <Modal.Body>
              <div className="padding15rem">
                  ...
                  <p className='span-text-dark' >Banner title</p>
                  <input type="text" className="form-input" placeholder='Banner Name' value={name} name="name" onChange={HandleInput} ></input>
                  </div>
                  <div className="inputwrapper">
                  <p className='span-text-dark' >redirection_link</p>
                  <input className="form-input" placeholder='Banner url' value={target} name="target" onChange={HandleInput}></input>
                  </div>
  ...
  <p ></p>
      ...
      <input className="form-input dropdown-toggle" value={Country.toString()} placeholder='Country Name ' data-bs-toggle="dropdown" aria-expanded="false" style={{width:"100%"}}></input>
      ...
      <div className="country-countainer d-flex  justify-content-between" style={{  height: 200, overflowY: 'auto'}}>
          ...
          <Modal.Footer>
            ...
            <button className='themeButton' onClick={editEventid ? gameEdit: eventupload}>
              {editEventid ?  "Edit Banner ":"Add Banner"}
            </button>
          </Modal.Footer>
        </Modal>
      </>
