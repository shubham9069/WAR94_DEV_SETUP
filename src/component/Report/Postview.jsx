...
getdata();
 ...
 <div className="userList" style={{margin:0,borderRadius:'12px 12px 0 0 '}} >
<img src={post[0]?.post_owner?.avatar} style={{width:150,height:150,borderRadius:6}}></img>

<div style={{flex:1}}>
<h6  style={{marginBottom:10,fontWeight:600,color:'#3F4254'}}>{post[0]?.post_owner?.name}

{post[0]?.post_owner?.kyc_status &&<svg width="21" height="21" viewBox="0 0 21 21" fill="none" xmlns="http://www.w3.org/2000/svg" style={{marginLeft:12}}>
...
{post[0]?.post_owner?.is_gold_memeber && <span className='span-box' style={{background:'#F8F5FF',color:'#5014D0',marginLeft:10}}>Pro user</span>}
</h6>
...
</svg>{post[0]?.post_owner?.username}
&nbsp;&nbsp;&nbsp;
...
</svg>+91 {post[0]?.post_owner?.mobile}
&nbsp;&nbsp;&nbsp;
...
<path fill-rule="evenodd" clip-rule="evenodd" d="M8.75065 7.00065C8.75065 7.96715 fo7.96715 8.75065 7.00065 8.75065C6.03415 8.75065 5.25065 7.96715 5.25065 7.00065C5.25065 6.03415 6.03415 5.25065 7.00065 5.25065C7.96715 5.25065 8.75065 6.03415 8.75065 7.00065ZM9.18847 8.92954C8.65402 9.53526 7.87195 9.91732 7.00065 9.91732C5.38982 9.91732 4.08398 8.61148 4.08398 7.00065C4.08398 5.38982 5.38982 4.08398 7.00065 4.08398C7.65726 4.08398 8.26319 4.30095 8.75066 4.66711C8.75077 4.34504 9.01189 4.08398 9.33399 4.08398C9.65615 4.08398 9.91732 4.34515 9.91732 4.66732C9.91732 6.15662 9.92093 7.24276 10.0851 7.96512C10.1654 8.31847 10.2692 8.50785 10.3662 8.60705C10.4435 8.68603 10.5568 8.75065 10.7923 8.75065C11.1145 8.75065 11.3757 9.01182 11.3757 9.33398C11.3757 9.65615 11.1145 9.91732 10.7923 9.91732C10.2987 9.91732 9.86513 9.76319 9.53224 9.4229C9.39065 9.27817 9.2784 9.11088 9.18847 8.92954Z" fill="#5E6278"/>
</svg>{post[0]?.post_owner?.email}
&nbsp;&nbsp;&nbsp;
...
<div>
                <h5 style={{fontWeight:900,color:'#3F4254',marginBottom:0}}>₹{post[0]?.post_owner?.winning}</h5>
                <p className="span-text-light" style={{fontSize:13,marginBottom:0}}>Total Earning </p>
            </div>
            {/* <div>
                <h5 style={{fontWeight:900,color:'#3F4254',marginBottom:0}}>{post?.post_owner?.followers}</h5>
                <p className="span-text-light" style={{fontSize:13,marginBottom:0}}>Followers</p>
            </div>
            <div>
                <h5 style={{fontWeight:900,color:'#3F4254',marginBottom:0}}>{post?.post_owner?.following}</h5>
...
<div  className="tab5-container" style={{marginTop:'1.5rem'}}>
<PostTab1 posts={post} single_user={ post.length ? post[0]?.post_owner : {}} setposts={setPost}/>
<div className=' ' style={{background:'white',borderRadius:12}}>
<div className='between-div padding15rem' >
<h6 className='section-heading'>Reported users</h6>
  <p style={{fontWeight:700,color:"#F1416C"}}>40 report </p>
</div>

{[...Array(4)]?.map((a)=>{


  return <div className='d-flex justify-content-between reported-user ' style={{gridGap:20,padding:'1rem 2rem'}}>
  <img src='images/users.png' style={{width:50,borderRadius:50}}></img>
  <div>
    <p className='span-text-dark' style={{margin:0}}>shubham 123</p>
    <span className='span-text-light' >shu123</span>
  </div>
  <div style={{flex:1}}>
  <span className='span-box' style={{background:"#D9DBE4",color:"black"}} >abusing</span>
  </div>
  <span className='span-box-light' style={{fontSize:10}}>20hrs</span>
</div>
})}
